...
const DROP_LINE = [
  ...
  // عناوين تسليم داخل سطر واحد لا تُنشر.
  /^\s*\**\s*(?:الهاشتاقات|الهاشتاجات|الوسوم)\s*\**\s*[:：]?\s*$/u,
  /^\s*\**\s*نص\s+بديل\s*\**\s*[:：]/u,
  // شرح الهاشتاقات («- واسعة: #عطور») توضيح للموظف لا جزء من المنشور.
  /^\s*[-•*]\s*[^:：\n#]{1,24}[:：]\s*#[\p{L}\p{N}_]+\s*$/u,
  /^\s*[-•]?\s*\**\s*(?:الموعد\s+المقترح|المؤشر|المصدر)\s*\**\s*[:：]/u,
];
...
const AFTER_POST =
  /^\s*(?:#{1,6}\s*)?\**\s*(?:ملاحظة\s+للمستخدم|ما\s+عملته|اللي\s+عملته|لماذا\s+اخترت|ليه\s+اخترت|الخطوة\s+التالية|الخطوات\s+التالية|قرارك|قرار\s+واحد|المتبقي|الباقي|رأيي|تقرير\s+الأثر|المصادر?|الصورة\s*\/?\s*(?:و\s*)?الفيديو|الوسائط(?:\s+المقترحة)?|المقترح\s+البصري|وصف\s+الصورة|نص\s+بديل|التوقيت\s+والقياس|القياس\s+والتوقيت|التوقيت\s+المقترح)\b/u;
